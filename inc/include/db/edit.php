...
// QUERY editar
// ----------------------------
$query  = "SELECT * FROM " . $arrDados['table'] . " WHERE $strKey";
$res    = my_query($query);
    ...
    switch ($dataField['type']) {
        ...
        case 'select':
            ...
            if (isset($dataField['select_options'])) {
                ...
                $key = $dataField['select_table']['key'];
                foreach ($arrSelect as $k => $v) {
                    if ($v[$key] == $fieldValue) {
        ...
        case 'file':
            ...
            <div action="' . $arrConfig['url_admin'] . '/include/db/upload.php?folder=' . $dataField['folder'] . '&width=' . $width . '&height=' . $height . '" class="dropzone" name="' . $fieldName . '" id="input' . $fieldName . '">
                ...
                init() {
                    ...
                    if (arrImgs[0] != 0) {
                        for (let i = 0; i < arrImgs.length; i++) {
                            ...
                            this.displayExistingFile(mockFile, "' . $arrConfig['url_fotos'] . '/' . $dataField['folder'] . '/" + arrImgs[i]);
...
<form name="formInserir" id="formInserir" action="<?= $arrConfig['url_admin'] . '/include/db/trata_editar.php?tabela=' . $arrDados['table'] ?>" method="post" enctype="multipart/form-data">
    <h5 class="form-header"><?= $arrDados['label'] ?></h5>
    <!-- <div class="form-desc">
        <?php
        //? echo $arrDados['desc'];
        ?>
    ...
    foreach ($arrDados['fields'] as $k => $v) {
        if ($v['edit']) {
            writeFieldForm($arrConfig, $v, $k, $res[0][$k]);
        }
    }
    ?>
