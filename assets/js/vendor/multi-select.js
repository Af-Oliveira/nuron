...
class IconicMultiSelect {
  ...
  _addOptionToList(option, index) {
    const html = `<span class="multiselect__selected" data-value="${
      option.value
    }">${
      this._tagTemplate
    ...
    const { lastElementChild: removeBtn } = this._multiselect.querySelector(
  ...
  _handleArrows(event) {
    if (event.keyCode === 40 || event.keyCode === 38) {
      ...
      const optionsContainerList = this._multiselect.querySelector(
        `.multiselect__options > ul`
      );
          ...
          ${
            this._options.length > 0 && this._itemTemplate
              ? this._options
                  .map((option, index) => {
                    return `
              <li data-value="${option.value}">${this._processTemplate(
                      this._itemTemplate,
                      index
                    )}</li>
            `;
  ...
  _showNoData(condition) {
    return condition
      ? `<p class="multiselect__options--no-data">${this._noData}</p>`
      : "";
  }
