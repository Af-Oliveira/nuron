  ...
  return class {
    ...
    _create(t) {
      ...
      (this._parent = new e(s)),
        (this._select = new e("div", { class: "multi-select__select" })),
        (this._selected_value = new e("span", {
          class: "multi-select__label",
        })),
        (this._optionsDiv = new e("div", { class: "multi-select__options" })),
      ...
      this._config.options.length == 0
        ? (this._select.get().innerHTML = this._config.noOptions)
        : false;
    }
    _generateOptionsOfSelect() {
      return (
        ...
        this._config.options.map((t) => {
          ...
          return this._optionsDiv.append(s.get()), s;
        })
      );
    }
    _handleClick(t) {
      if (
        this._config.selected_options.size <
          this._config.max_selected_options ||
        this._config.max_selected_options == null
      ) {
        if (
          (this._firstActive(),
          t.stopPropagation(),
          this._closeAllLists(),
          "multi-select__autocomplete" !== t.target.className)
        ) {
      ...
      if ("fa " + t.target.parentElement.classList[1] !== this._config.icon) {
        if ("I" !== t.target.tagName && "svg" !== t.target.tagName) {
          if (
            this._config.selected_options.size <
              this._config.max_selected_options ||
            this._config.max_selected_options == null
          ...
          this._config.selected_options.delete(
            t.target.getAttribute("data-value")
          );
          this._unselectOption(t.target.getAttribute("data-value"));
        }
      } else {
        this._unselectOption(t.target.parentElement.getAttribute("data-value"));
      }
    }
    _setValue(t, e, s) {
      console.log("BIG CLOWN");
      console.log({
        t,
        e,
        s,
      ...
      if (
        ...
        this._options.forEach((t) => {
          t.removeClass("multi-select__option--selected");
        }),
        this._config.multiple)
      ) {
        const t = this._config.value.map((t) => {
          const e = this._config.options.find((e) => e.value === t);
          this._config.selected_options.set(e.value, e.label);
          return (
            this._options
              .find(
                (t) => t.get().getAttribute("data-value") === e.value.toString()
              )
              .addClass("multi-select__option--selected"),
            e
          );
        });
      ...
      if ("" !== i) {
        this._options
          .find(
            (t) => t.get().getAttribute("data-value") === i.value.toString()
          )
          .addClass("multi-select__option--selected"),
    ...
    _selectOption(t, e) {
      (this._selectedOption = t),
        this._selected_value.setText(t.label),
    ...
    _selectOptions(t, s) {
      this._selected_value.setText(""),
        (this._icons = t.map((t) => {
          ...
          return (
            s.append(i.get()), this._selected_value.append(s.get()), i.get()
          );
        })),
        s &&
          this._optionsDiv.setTop(
    ...
    _unselectOption(t) {
      ...
      -1 !==
        (s = t.target
          ? e.indexOf(t.target.getAttribute("data-value"))
          : e.indexOf(t)) && e.splice(s, 1),
        this._setValue(e, !0, !0);
    }
    _sortOptions(t) {
      this._options.forEach((e) => {
        e.get().textContent.toLowerCase().includes(t.target.value.toLowerCase())
          ? e.removeClass("multi-select__option--hidden")
          : e.addClass("multi-select__option--hidden");
      }),
        this._firstActive();
    }
    _visibleOptions() {
      return this._optionsDiv
        .get()
        .querySelectorAll(
          "div.multi-select__option:not(.multi-select__option--hidden)"
        );
    }
    _unselectedOptions() {
      return this._optionsDiv
        .get()
        .querySelectorAll(
          "div.multi-select__option:not(.multi-select__option--selected):not(.multi-select__option--hidden)"
        );
    }
    _selectedOptions() {
      return this._optionsDiv
        .get()
        .querySelectorAll(".multi-select__option--selected");
    }
    _arrows(t) {
      let e = this._visibleOptions();
      40 == t.keyCode
        ? (this._nextUnselected(e), this._addActive(e))
        : 38 == t.keyCode
        ? (this._previousUnselected(e), this._addActive(e))
        : (13 != t.keyCode && 9 != t.keyCode) ||
          (t.preventDefault(),
          this.currentFocus > -1 &&
            (e && e[this.currentFocus].click(), this._removeActive(e)));
    }
    _addActive(t) {
      if (!t) return !1;
      this._removeActive(t),
        this._listEndingCheck(t),
        t[this.currentFocus].classList.add("multi-select__option-active"),
        t[this.currentFocus].scrollIntoView();
    }
    _removeActive(t) {
      for (var e = 0; e < t.length; e++)
        t[e].classList.remove("multi-select__option-active");
    }
    _nextUnselected(t) {
      for (
        this.currentFocus++;
        t[this.currentFocus] &&
        t[this.currentFocus].classList.contains(
    ...
    _listEndingCheck(t) {
      this.currentFocus >= t.length &&
        ((this.currentFocus = -1), this._nextUnselected(t)),
        this.currentFocus < 0 &&
          ((this.currentFocus = t.length), this._previousUnselected(t));
    }
    _firstActive() {
      (this.currentFocus = -1),
        this._removeActive(this._visibleOptions()),
        this._config.autocomplete &&
          this._unselectedOptions().length > 0 &&
          (this._nextUnselected(this._visibleOptions()),
          this._visibleOptions()[this.currentFocus].classList.add(
            "multi-select__option-active"
          ));
    }
    _closeAllLists() {
      let t = document.getElementsByClassName("multi-select__select");
      for (let e = 0; e < t.length; e++)
        t[e] !== this._select.get() &&
          t[e].classList.contains("multi-select__select--opened") &&
          t[e].classList.remove("multi-select__select--opened");
    }
  };
})();
